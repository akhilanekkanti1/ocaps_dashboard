...
### **64%** of students report at least one experience of physical abuse.{data-commentary-width=600}



### Abuse experiences may be related to missing school.{data-commentary-width=600}
```{r}
...
parent_liklong <- parent_lik %>% 
 ...
 sum_table <- parent_liklong %>%
  ...
  select(question, n, category) %>% 
   mutate(question = stringr::str_wrap(question, width = 35)) %>% 
  group_by(question) %>%
  mutate(total = sum(n)) %>% 
  ungroup() %>% 
  mutate(pct = (n/total))
...
ggplot(aes(question, pct)) + 
  ...
  geom_abline(aes(intercept = 0, slope = 0), 
               ...
               linetype = 2) +
  ylim(-0.25, 1) +
  labs(title = str_wrap("Students Report on their Parents' Engagement", width = 30),
        y = NULL,
        subtitle = str_wrap("Among those who experienced at least one type of physical abuse, many report their parents are engaging and present.", width = 40)
  ) +
  theme(
...
dq %>% 
  ...
  facet_grid(~age) +
  labs(
    title = "Suicidality in the past 12 months")
...
### School {data-commentary-width=400}
...
ggplot(radar, aes(x = told)) +
  ...
  coord_polar() + 
  scale_fill_manual(values = c("brown", "#31a354")) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 8, color = "sienna2")
  )
