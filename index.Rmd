...
dq <- dq %>% 
  mutate_at(vars(Q3:Q102), as.factor) %>% #new dyplr: mutate(across(c(Q3:Q102)), as.factor)
...
# This should work instead of lines 51-66
dq1 <- dq1 %>% 
  mutate(across(c(Q120, Q123, Q126, Q129, Q132, Q135, Q138, Q141, Q144, Q147, Q153, Q162, Q165, Q174, Q346), ~as.numeric(as.character(.))))
...
d <- d %>% 
  mutate(dmiss = forcats::fct_recode(Q18, !!!days),
         dmiss = as.numeric(dmiss), drop(NA),
         PATOTR = as.numeric(PATOTR), drop(NA)) # you can add all htese to one mutate() function
...
d <- d %>% 
  mutate(across(c(Q87A, Q88A, Q89A, Q90A, Q91A, Q92A, Q93A, Q94A, Q95A, Q97A, Q100A, Q101A, Q104A), ~as.numeric(as.character(.))))
...
# Do the school names need to be anonymized? If so, you can move this to your "d" folder.
d <- d %>% 
...
d %>% 
  ...
  mutate(Q18 = factor(Q18, levels = c("1-2 days", "3-5 days", "6-10 days", "11-15 days", "16 or more days", "None","I choose not to answer")),
         Q18 = fct_rev(Q18))  %>% # This should display the levels in the way you specify above
ggplot(aes(Q18, fill = loc)) +
  ...
  theme(plot.title.position = "plot") # I am hoping this left-justifies the title
...
x <- dq1 %>% 
  # See if the code below works. You can use "if_all" or "if_any" and I think you wanted "if_all"
  filter(if_all(c(Q124, Q127, Q130, Q133, Q136, Q139, Q142, 
                  Q145, Q148, Q15, Q163, Q166, Q175), ~ .x != "Not in the past year")) %>% 
...
```{r}
#You want to reorder the levels in this plot to match the plot that is adjacent to it. I like fct_relevel() to just reorder manually.

ggpubr::ggballoonplot(school1, fill = "value") + 
  ...
  labs(
    ...
    y = "Missed Days of School", # I don't see the x and y labels in the figure
    x = "Grades"
  ) +
  theme(plot.title.position = "plot") # I am hoping this left-justifies the title
...
parent_lik <- d %>% 
  filter(Q87A == 1 | #see the filter code on line 252
         Q88A == 1 | 
        Q89A == 1 |
        Q90A == 1 |
        Q91A == 1 |
...
Q136) %>% 
  drop_na() %>% 
  mutate(across(cols = starts_with("Q"), factor(levels = liklevel, ordered = TRUE))) #You DO know of the across() function, nice job!
...
parent_liklong <- parent_lik %>% 
  ...
  pivot_longer(
    cols = contains("."), #or you can use the everything() helper function
    values_to = "category",
    names_to = "question",
    values_drop_na = TRUE
    ) %>% 
...
ggplot(aes(question, pct)) + 
  ...
  geom_abline(aes(intercept = 0, slope = 0), 
               color = "grey40", # maybe try "white" here
               linetype = 2) +
  ylim(-0.25, 1) +
...
dq1 %>% 
  ...
  theme(axis.text.x = element_text(angle = 45), #maybe easier to read at an angle?
        legend.position = "none",
        plot.title.position = "plot",
        panel.background = element_blank()) +
